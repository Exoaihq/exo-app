...
function ChatHistory({ history, loading }: { history: ChatMessage[], loading: boolean }) {
        ...
        <div className="relative w-full p-6 overflow-y-auto h-[40rem]">
            ...
            <ul className="space-y-2 h-full">
                {history.map((item, index) => {
                    ...
                    const line = item.role === ChatUserType.assistant ? <li className="flex justify-start" key={index}>
                        <div className="relative max-w-xl px-4 py-2 text-gray-700 rounded shadow">
                            <span className="block">{item.content}
                            </span>
                        </div>
                    </li> : <li className="flex justify-end" key={index}>
                ...
                {loading && <li className="flex justify-start" >
                    <div className="relative max-w-xl px-4 py-2 text-gray-700 rounded shadow">
